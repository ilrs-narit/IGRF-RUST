...
| P1 Offline kiosk | 1 | Disconnect network, boot microSD. App opens stopped in fullscreen; verify X11/GPU rendering, 1024×600 mode, USB touch edges, Onboard typing and GTK file selection. Record the now-known panel model. Close/reopen dialogs during mock activity and check control timing separately. If the digitizer cannot sense the top edge, raise `Display.TopInset` in `SystemConfig.json` (README's Display table) and re-test the tab strip. |
