...
use igrf_io::{
    ...
    StoredTle, TleFilter, TleStore, DEFAULT_BIND_ADDRESS,
};
...
impl IgrfApp {
    ...
    fn show_logs_folder_list(&mut self, ui: &mut egui::Ui) {
                ...
                egui::Grid::new("saved-log-files")
                    ...
                    .show(ui, |ui| {
                        for row in &self.log_files {
                            let selected =
                                self.log_files_sel.as_deref() == Some(row.name.as_str());
                            if ui
                                .selectable_label(
                                    selected,
        ...
        if let Some(name) = pick {
            self.log_files_sel = (self.log_files_sel.as_deref() != Some(name.as_str()))
                .then_some(name);
        }
    }
    ...
    fn show_external_drive_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            let at_root = self
                ...
                .is_none();
            if ui.add_enabled(!at_root, egui::Button::new("Back")).clicked() {
                    ...
                    .show(ui, |ui| {
                        for row in &self.ext_entries {
                            if row.is_dir {
                                ...
                                let selected =
                                    self.ext_sel.as_deref() == Some(row.name.as_str());
                                if ui
                                    .selectable_label(
                                        selected,
                                        egui::RichText::new(&row.name).monospace(),
    ...
    fn refresh_drives(&mut self) {
        ...
        if let Some(cwd) = self.ext_cwd.clone() {
            if !self.ext_drives.iter().any(|drive| cwd.starts_with(&drive.root)) {
                self.ext_cwd = None;
